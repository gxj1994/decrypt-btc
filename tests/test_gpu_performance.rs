...
fn test_performance_stats_basic() {
    ...
    for i in 1..=12 {
        config
            .word_positions
            .insert(format!("word{}", i - 1), vec!["abandon".to_string()]);
    }
...
fn test_performance_stats_multiple_work_items() {
    ...
    for i in 1..=12 {
        if i <= 4 {
            config
                .word_positions
                .insert(format!("word{}", i - 1), test_words.clone());
        } else {
            config
                .word_positions
                .insert(format!("word{}", i - 1), vec!["abandon".to_string()]);
...
fn test_performance_speed_display() {
    ...
    for (speed, expected_unit) in test_cases {
        ...
        println!(
            "速度: {:.0} -> 显示: {} (预期: {})",
            speed, display, expected_unit
        );
        assert!(
            display.contains(expected_unit.split_whitespace().last().unwrap()),
            "速度单位应该正确显示"
        );
    }
