...
#define MNEMONIC_SIZE 12  // 默认12词，可被search.cl等覆盖
#endif
...
// BIP32 派生路径: m/44'/0'/0'/0/0 (比特币Legacy标准路径)
// 44' = 0x8000002C (BIP44)
// 0'  = 0x80000000 (比特币币种)
// 0'  = 0x80000000 (账户)
...
// BIP32 派生路径表（4 种地址类型，索引与 ADDRESS_TYPE 宏一致）:
// 索引0: m/86'/0'/0'/0/0 (BIP86 Taproot)
// 索引1: m/44'/0'/0'/0/0 (BIP44 Legacy)
// 索引2: m/49'/0'/0'/0/0 (BIP49 Nested SegWit)
// 索引3: m/84'/0'/0'/0/0 (BIP84 Native SegWit)
__constant uint DERIVATION_PATHS[4][5] = {
    {0x80000056, 0x80000000, 0x80000000, 0x00000000, 0x00000000},  // 86' Taproot
    {0x8000002C, 0x80000000, 0x80000000, 0x00000000, 0x00000000},  // 44' Legacy
    {0x80000031, 0x80000000, 0x80000000, 0x00000000, 0x00000000},  // 49' Nested SegWit
    {0x80000054, 0x80000000, 0x80000000, 0x00000000, 0x00000000}   // 84' Native SegWit
};
...
inline void mod_add_n_mnemonic(const mp_number* a, const mp_number* b, mp_number* result) {
    ...
    // 如果溢出或结果 >= n，则减去 n
    if (carry || mp_cmp_n(&temp, &n_local) >= 0) {
        mp_word borrow = 0;
        for (int i = 0; i < 8; i++) {
            mp_word diff = temp.d[i] - n_local.d[i] - borrow;
            // 正确的借位检测
            if (borrow == 0) {
                borrow = (temp.d[i] < n_local.d[i]) ? 1 : 0;
            } else {
                borrow = (temp.d[i] <= n_local.d[i]) ? 1 : 0;
            }
            result->d[i] = diff;
...
inline void derive_child_key(const uchar parent_key[64], uint index, uchar child_key[64]) {
    ...
    if (index >= 0x80000000) {
        // 硬化派生: 使用 0x00 || 父私钥 || 索引
        data[0] = 0x00;
        // 逐字节复制 32 字节私钥，避免未对齐的 ulong 指针双关
        for (int j = 0; j < 32; j++) {
            data[1 + j] = parent_key[j];
        }
    } else {
        // 普通派生: 使用 压缩父公钥 || 索引
        // 需要先计算父公钥
        ...
        // 逐字节复制，避免未对齐的 ulong 指针双关
        for (int j = 0; j < 32; j++) {
            data[1 + j] = parent_public[1 + j];
        }
    }
    ...
    // 输出子链码 (后32字节) - 逐字节复制 HMAC 右半部分
    for (int j = 0; j < 32; j++) {
        child_key[32 + j] = hmac_result[32 + j];
    }
}
...
inline void derive_path(const seed_t* seed, __constant const uint* path, uint path_len, uchar private_key[32]) {
    ...
    // 取前32字节作为私钥 (逐字节复制，避免未对齐的 ulong 指针双关)
    for (int j = 0; j < 32; j++) {
        private_key[j] = current_key[j];
    }
}

// 获取比特币Legacy私钥 (标准派生路径 m/44'/0'/0'/0/0)
inline void get_bitcoin_legacy_private_key(const mnemonic_t* mnemonic, uchar private_key[32]) {
    seed_t seed;
...
inline void get_bitcoin_legacy_private_key_local(const local_mnemonic_t* mnemonic, uchar private_key[32]) {
    mnemonic_t mn;
    for (int i = 0; i < 24; i++) {
        mn.words[i] = mnemonic->words[i];
    }
    get_bitcoin_legacy_private_key(&mn, private_key);
}
