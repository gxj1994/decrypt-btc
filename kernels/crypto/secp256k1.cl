...
/* ------------------------------------------------------------------------ */
/* 预计算基点 G 的倍数表 (窗口大小为4，15个点)                               */
/* PRECOMPUTED_G[i] = (i+1) * G, i = 0..14                                */
/* 存储 1G ~ 15G，直接使用窗口值作为索引                                     */
/* 使用 secp256k1 库计算生成                                                */
/* ------------------------------------------------------------------------ */
// 预计算表数据 - 1G ~ 15G
#define MP_NUM(a0,a1,a2,a3,a4,a5,a6,a7) {{a0,a1,a2,a3,a4,a5,a6,a7}}

__constant const point PRECOMPUTED_G[15] = {
    // 1*G
    ...
    // 2*G
    {MP_NUM(0x5c709ee5, 0xabac09b9, 0x8cef3ca7, 0x5c778e4b, 0x95c07cd8, 0x3045406e, 0x41ed7d6d, 0xc6047f94),
     MP_NUM(0x50cfe52a, 0x236431a9, 0x3266d0e1, 0xf7f63265, 0x466ceaee, 0xa3c58419, 0xa63dc339, 0x1ae168fe)},
    // 3*G
    ...
    // 4*G
    {MP_NUM(0xe8c4cd13, 0x74fa94ab, 0x0ee07584, 0xcc6c1390, 0x930b1404, 0x581e4904, 0xc10d80f3, 0xe493dbf1),
     MP_NUM(0x47739922, 0xcfe97bdc, 0xbfbdfe40, 0xd967ae33, 0x8ea51448, 0x5642e209, 0xa0d455b7, 0x51ed993e)},
    // 5*G
    ...
    // 6*G
    {MP_NUM(0x60297556, 0x2f057a14, 0x8568a18b, 0x82f6472f, 0x355235d3, 0x20453a14, 0x755eeea4, 0xfff97bd5),
     MP_NUM(0xb075f297, 0x3c870c36, 0x518fe4a0, 0xde80f0f6, 0x7f45c560, 0xf3be9601, 0xacfbb620, 0xae12777a)},
    // 7*G
    ...
    // 8*G
    {MP_NUM(0xe10a2a01, 0x67784ef3, 0xe5af888a, 0x0a1bdd05, 0xb70f3c2f, 0xaff3843f, 0x5cca351d, 0x2f01e5e1),
     MP_NUM(0x6cbde904, 0xb5da2cb7, 0xba5b7617, 0xc2e213d6, 0x132d13b4, 0x293d082a, 0x41539949, 0x5c4da8a7)},
    // 9*G
    ...
    // 10*G
    {MP_NUM(0x47e247c7, 0x52a68e2a, 0x1943c2b7, 0x3442d49b, 0x1ae6ae5d, 0x35477c7b, 0x47f3c862, 0xa0434d9e),
     MP_NUM(0x037368d7, 0x3cbee53b, 0xd877a159, 0x6f794c2e, 0x93a24c69, 0xa3b6c7e6, 0x5419bc27, 0x893aba42)},
    // 11*G
    ...
    // 12*G
    {MP_NUM(0x70afe85a, 0xc5b0f470, 0x9620095b, 0x687cf441, 0x4d734633, 0x15c38f00, 0x48e7561b, 0xd01115d5),
     MP_NUM(0xf4062327, 0x6b051b13, 0xd9a86d52, 0x79238c5d, 0xe17bd815, 0xa8b64537, 0xc815e0d7, 0xa9f34ffd)},
    // 13*G
    ...
    // 14*G
    {MP_NUM(0x60e823e4, 0xe49b241a, 0x678949e6, 0x26aa7b63, 0x07d38e32, 0xfd64e67f, 0x895e719c, 0x499fdf9e),
     MP_NUM(0x03a13f5b, 0xc65f40d4, 0x7a3f95bc, 0x464279c2, 0xa7b3d464, 0x90f044e4, 0xb54e8551, 0xcac2f6c4)},
    // 15*G
...
void scalar_mult_base_jacobian_windowed(const uchar scalar[32], uchar result[65]) {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        // 如果窗口值非零，加上对应的预计算点
        if (high_nibble != 0) {
            // 直接使用窗口值-1作为索引（0-based，PRECOMPUTED_G[0]=1G）
            if (jacobian_is_infinity(&r)) {
                affine_to_jacobian_c(&r, &PRECOMPUTED_G[high_nibble - 1]);
            } else {
                jacobian_add_affine_c(&r, &r, &PRECOMPUTED_G[high_nibble - 1]);
            }
        }
        ...
        if (low_nibble != 0) {
            // 直接使用窗口值-1作为索引（0-based，PRECOMPUTED_G[0]=1G）
            if (jacobian_is_infinity(&r)) {
                affine_to_jacobian_c(&r, &PRECOMPUTED_G[low_nibble - 1]);
            } else {
                jacobian_add_affine_c(&r, &r, &PRECOMPUTED_G[low_nibble - 1]);
            }
        }
    }
...
void private_to_public(const uchar private_key[32], uchar public_key[65]) {
    // 使用窗口优化版本
    scalar_mult_base_jacobian_windowed(private_key, public_key);
}
